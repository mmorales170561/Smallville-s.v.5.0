...
# --- 1. GLOBAL UI CONFIG ---
st.set_page_config(page_title="RUBY-OPERATOR v8.1", layout="wide")
...
# --- 2. PERSISTENCE ---
if 'last_log' not in st.session_state: st.session_state.last_log = "SYSTEM FORCED ONLINE."
...
# --- 5. MAIN HUD ---
st.title("🏹 SMALLVILLE S.V. 8.1")
# Explicitly defining tabs to ensure they render
t1, t2, t3, t4, t5 = st.tabs(["🚀 STRIKE", "📊 MATRIX", "📟 HUD", "🛠️ TERMINAL", "🔍 DEBUG"])

with t1:
    st.subheader("🎯 TARGET ACQUISITION")
    st.text_input("ENTER TARGET", key="target_input", value=st.session_state.target)
    st.button("🔥 INITIATE FULL SCAN")
