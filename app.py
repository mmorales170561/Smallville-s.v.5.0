...
def master_boot():
    if 'term_logs' not in st.session_state: 
        st.session_state.term_logs = "[*] SYSTEM ONLINE\n[18:33:14] ALERT: Nuclei detected Exposed .git directory\n[18:34:33] STRIKE COMPLETE: IDOR found."
...
with t1:
    ...
    if st.button("📦 RUN GIT-DUMPER SIM"):
        ts = datetime.now().strftime('%H:%M:%S')
        st.session_state.term_logs += f"\n[{ts}] git-dumper https://api.target.com/.git/ ./output"
        st.session_state.term_logs += f"\n[{ts}] SUCCESS: Downloaded 142 objects. Found config/index."
        st.session_state.term_logs += f"\n[{ts}] SECRET FOUND: DB_PASSWORD=Krypton2026!"
        st.rerun()
...
with t4:
    ...
    if st.session_state.loot_items:
        # FIXED SELECTBOX LOGIC
        options = range(len(st.session_state.loot_items))
        bug_idx = st.selectbox("Select Finding", options, 
                               format_func=lambda x: f"{st.session_state.loot_items[x].get('path')} ({st.session_state.loot_items[x].get('severity', 'P1')})")
        
        l = st.session_state.loot_items[bug_idx]
        
        # FIXED: Properly terminated triple-quoted f-string
...
## Impact*:
{l.get('impact', 'Unauthorized access to data.')}
...
* **Severity:** {l.get('severity', 'P1')}
* **Timestamp:** {l.get('ts')}"""
        
        st.code(report_text, language="markdown")
    else:
        st.info("No loot available.")
