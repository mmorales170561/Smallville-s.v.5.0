...
# --- 1. HUD CONFIG ---
st.set_page_config(page_title="SMALLVILLE T2 V9.8", layout="wide")
...
if 'term_logs' not in st.session_state: st.session_state.term_logs = "VAULT ONLINE..."
...
def forge_arsenal():
    ...
    for name, url in repos.items():
        try:
            ...
            if name == "secretfinder": subprocess.run(["pip", "install", "-r", f"{tp}/requirements.txt"], capture_output=True)
            status.write(f"✅ {name.upper()} REPO")
        except: status.write(f"❌ {name.upper()} REPO FAIL")
    
    # Default Bug-Hunting Wordlist
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("📚 WORDLIST MANAGER")
    up_file = st.file_uploader("Upload List (.txt)", type="txt")
    if up_file:
        with open(os.path.join(WORD_DIR, up_file.name), "wb") as f: f.write(up_file.getbuffer())
        st.success(f"Stored: {up_file.name}")
    
    current_lists = os.listdir(WORD_DIR)
    if current_lists: st.caption(f"Active Lists: {', '.join(current_lists)}")
...
with t3:
    ...
    if loot_files:
        ...
        c2.download_button("📦 ZIP ALL", buf.getvalue(), file_name="loot.zip", mime="application/zip")
...
with t4:
    ...
    if cols[3].button("🕰️ GAU"): st.session_state.c_input = f"gau {tgt} --o {LOOT_DIR}/urls.txt"
    ...
    if st.button("🚀 EXECUTE"):
        cmd = c_in
        if "subfinder" in c_in: cmd = c_in.replace("subfinder", f"{BIN_DIR}/subfinder")
        if "nuclei" in c_in: cmd = c_in.replace("nuclei", f"{BIN_DIR}/nuclei")
        if "ffuf" in c_in: cmd = c_in.replace("ffuf", f"{BIN_DIR}/ffuf")
        if "gau" in c_in: cmd = c_in.replace("gau", f"{BIN_DIR}/gau")
