...
INITIAL_STATE = {
    'target': "example.com",
    'last_log': "SYSTEM ONLINE. ARSENAL V5.1 READY.",
...
# --- 3. HUD CONFIG ---
st.set_page_config(page_title="RUBY-OPERATOR v5.1", layout="wide")
st.markdown("""
    <style>
...
TOOL_URLS = {
    ...
    "arjun": "https://github.com/s0md3v/Arjun/archive/refs/tags/v2.2.1.zip",
    ...
    "dirsearch": "https://github.com/maurosoria/dirsearch/archive/master.zip",
...
# --- 5. ENHANCED CORE ENGINES ---
def find_exe(name):
    """Deep search for binary, python script, or system tool."""
    # Priority 1: Direct Binary or .py in /tmp/ruby_bin
    for root, _, files in os.walk(BIN_DIR):
        for f in files:
            if f == name or f == f"{name}.py":
                ...
                os.chmod(p, 0o755)
                return p
    # Priority 2: System installed tools (nmap, curl, etc.)
    return shutil.which(name)

def fabricate_core(tool_name):
    """Unpacks anything and flattens it into the bin directory."""
...
with t1: # STRIKE OPS
    ...
    if auth:
        ...
        if st.button("🔥 INITIATE TARGET-SPECIFIC STRIKE"):
            st.session_state.last_log = f"🚀 [INIT] {st.session_state.battery_type} ATTACK STARTING...\n"
            # Strike logic runs here...
    else: st.error(msg)

with t2: # ARSENAL MATRIX
    st.header("📋 STATUS MATRIX (v5.1)")
    cols = st.columns(4)
