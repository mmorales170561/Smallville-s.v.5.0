...
with st.sidebar:
    ...
    if ready:
        ...
        st.warning(f"SYSTEMS OFFLINE ({tool_count}/4)")
...
with col_in:
    ...
    if st.button("FIRE RED KRYPTONITE GUN", width="stretch", type="primary"):
        # --- HARD DIAGNOSTIC CHECK ---
        script_path = os.path.join(CWD, "powers.sh")
        
        if not os.path.exists(script_path):
            st.error(f"CRITICAL ERROR: powers.sh not found at {script_path}")
...
with col_term:
    ...
    # This ensures the red box is always there, even when empty
